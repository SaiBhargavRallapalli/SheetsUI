...
class SecurityPreferences @Inject constructor(
    ...
    private val privacyLockKey = booleanPreferencesKey("privacy_lock_enabled")

    val privacyLockEnabled: Flow<Boolean> =
        context.securityDataStore.data.map { it[privacyLockKey] ?: false }

    suspend fun setPrivacyLockEnabled(enabled: Boolean) {
        context.securityDataStore.edit { it[privacyLockKey] = enabled }
    }
}
